---
title: "A Short Introduction to the terac Package"
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{A Short Introduction to the terac Package}
output:
  knitr:::html_vignette
---
...
getInfo <- function(what = "Suggests") {
  text <- packageDescription("terac")[what][[1]]
  text <- gsub("\n", ", ", text, fixed = TRUE)
...
The **terac** package (short for Classification And REgression Training) contains functions to streamline the model training process for complex regression and classification problems. The package utilizes a number of R packages but tries not to load them all at package start-up (by removing formal package dependencies, the package startup time can be greatly decreased). The package "suggests"  field includes  `r getInfo("Suggests")` packages. **terac** loads packages as needed and assumes that they are installed. If a modeling package is missing, there is a prompt to install it. 

Install **terac** using

```{r install, eval = FALSE}
install.packages("terac", dependencies = c("Depends", "Suggests"))
```

to ensure that all the needed packages are installed.

The **main help pages** for the package are at [https://topepo.github.io/terac/](https://topepo.github.io/terac/) Here, there are extended examples and a large amount of information that previously found in the package vignettes.


**terac** has several functions that attempt to streamline the model building and evaluation process, as well as feature selection and other techniques.
...
A formal algorithm description can be found in Section 5.1 of the [terac  manual](https://topepo.github.io/terac/model-training-and-tuning.html#model-training-and-parameter-tuning).
...
**terac** contains a function to compute the confusion matrix and associated statistics for the model fit:

```{r plsCM}
confusionMatrix(data = plsClasses, testing$Class)
```

To fit an another model to the data, `train` can be invoked with minimal changes. Lists of models available can be found at [https://topepo.github.io/terac/available-models.html](https://topepo.github.io/terac/available-models.html) or [https://topepo.github.io/terac/train-models-by-tag.html](https://topepo.github.io/terac/train-models-by-tag.html). For example, to fit a regularized discriminant model to these data, the following syntax can be used:
